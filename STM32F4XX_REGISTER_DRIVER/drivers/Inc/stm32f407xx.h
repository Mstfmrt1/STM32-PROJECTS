...
// -------------------REGISTER OF A GPIO PERIPHERAL FOR STM32F407----------------------
typedef struct{
	...
	volatile uint32_t AFR[2];  // GPIO alternate function AFR[0]low-AFR[1]high register Address offset: 0x20-0x24
...
typedef struct{
	...
	uint32_t RESERVED1[2]; // GPIO alternate function AFR[0]low-AFR[1]high register 		   Address offset: 0x28-0X2C
...
typedef struct
{
	...
	volatile uint32_t EXTICR[4];    // SYSCFG external interrupt configuration register1 Address offset: 0x08-0x14
	uint32_t      RESERVED1[2];  //SYSCFG external interrupt configuration register 2	 Reserved, 0x18-0x1C
	volatile uint32_t CMPCR;        // SYSCFG external interrupt configuration register3 Address offset: 0x20
	uint32_t      RESERVED2[2];  //  SYSCFG external interrupt configuration register4   Reserved, 0x24-0x28
	volatile uint32_t CFGR;         // Compensation cell control register                Address offset: 0x2C
} SYSCFG_RegDef_t;
...
#define GPIOJ_REG_RESET()               do{ (RCC->AHB1RSTR |= (1 << 7)); (RCC->AHB1RSTR &= ~(1 << 9)); }while(0)
#define GPIOK_REG_RESET()               do{ (RCC->AHB1RSTR |= (1 << 8)); (RCC->AHB1RSTR &= ~(1 << 10)); }while(0)
