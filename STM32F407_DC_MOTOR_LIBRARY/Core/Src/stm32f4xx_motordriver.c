...
#include "stm32f4xx_motordriver.h"
#include "main.h"
...
void DC_MOTOR_Init(uint8_t MOTOR_Instance)
{
	...
	/*ARR_Value = 1;
	for(i=0; i<DC_MOTOR_CfgParam[MOTOR_Instance].PWM_RES_BITS; i++)
	{
		ARR_Value *= 2;
	}
	PSC_Value = (uint32_t) ((DC_MOTOR_CfgParam[MOTOR_Instance].TIM_CLK_MHz*1000000) / (ARR_Value*DC_MOTOR_CfgParam[MOTOR_Instance].PWM_FREQ_Hz));
	PSC_Value--;
	ARR_Value -= 2;*/

	//--------[ Configure The DC Motor PWM Timer Channel ]------

	/*htim.Instance = DC_MOTOR_CfgParam[MOTOR_Instance].TIM_Instance;
	...
	HAL_TIM_PWM_ConfigChannel(&htim4, &sConfigOC, DC_MOTOR_CfgParam[MOTOR_Instance].PWM_TIM_CH);
...
/*uint32_t DC_MOTOR_Get_MaxFreq(uint8_t MOTOR_Instance)
{
	...
	return ((DC_MOTOR_CfgParam[MOTOR_Instance].TIM_CLK_MHz*1000000)/ARR_Value);
}*/
